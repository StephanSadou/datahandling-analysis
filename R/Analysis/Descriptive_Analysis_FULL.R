...
# Set working directory to where your data_stage CSVs are (change if needed)
...
# ---------------------------
# 1. Load data (from your files)
# ---------------------------
# FAOSTAT
faostat <- read.csv("FAOSTAT_stage.csv", header = TRUE) %>%
  filter(Item == "Sugar cane")

# World Bank
wb <- read.csv("wb_stage.csv", header = TRUE) %>%
  mutate(Year = as.integer(date))

# NASA
nasa <- read.csv("nasa_stage.csv", header = TRUE)

# ---------------------------
# 2. Prepare sugarcane yield
# ---------------------------
faostat_yield <- faostat %>%
  filter(Item == "Sugar cane", Element == "Yield") %>%
  select(Year, Item, Yield = Value, Unit)

# Ensure Year is integer
faostat_yield$Year <- as.integer(faostat_yield$Year)

# ---------------------------
# 3. Prepare NASA seasonal classification.
#    Assumptions are as follows: Plantation (Jul–Sep), Growth (Oct–Mar), Maturation (Apr–Jun), Harvest flag
# ---------------------------
nasa_seasons <- nasa %>%
  rename(
    Date = date,
  ...
  mutate(
    ...
    Harvest = ifelse(Month %in% 7:12, 1, 0)
  ) %>%
  select(Year, Month, Season, Harvest, Temperature, Rainfall, Humidity, SolarRadiation) %>%
  filter(!is.na(Season))

# ---------------------------
# 4. Aggregate climate by Year & Season
# ---------------------------
nasa_annual <- nasa_seasons %>%
  group_by(Year, Season) %>%
  summarise(
    Total_Rainfall = sum(Rainfall, na.rm = TRUE),
    Avg_Temperature = mean(Temperature, na.rm = TRUE),
    Avg_Humidity = mean(Humidity, na.rm = TRUE),
    Avg_Solar = mean(SolarRadiation, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_wider(
    names_from = Season,
    values_from = c(Total_Rainfall, Avg_Temperature, Avg_Humidity, Avg_Solar),
    names_sep = "_"
  )

# ---------------------------
# 5. Merge climate + yield + WB
# ---------------------------
df_yield <- faostat_yield %>%
  left_join(nasa_annual, by = "Year") %>%
  left_join(wb %>% select(Year, value), by = "Year") %>%
  rename(Agri_GDP_Share = value)   # world bank file uses 'value' column for indicator

# Remove rows with missing essential data
df_yield <- df_yield %>% filter(!is.na(Yield))

# ---------------------------
# 6. Basic checks & housekeeping
# ---------------------------
...
lagged_df_yield <- df_yield %>%
  arrange(Year) %>%
  mutate(
    Rainfall_Plantation_lag1 = lag(Total_Rainfall_Plantation, 1),
    Temp_Plantation_lag1     = lag(Avg_Temperature_Plantation, 1),
    Rainfall_Growth_lag1     = lag(Total_Rainfall_Growth, 1),
    Temp_Growth_lag1         = lag(Avg_Temperature_Growth, 1),
    Rainfall_Maturation_lag1 = lag(Total_Rainfall_Maturation, 1),
    Temp_Maturation_lag1     = lag(Avg_Temperature_Maturation, 1)
  )

# Save lagged snapshot
fwrite(lagged_df_yield, "results/df_yield_with_lags.csv")

# Correlation heatmaps for lagged variables (Pearson & Spearman)
lagged_variables <- c("Rainfall_Plantation_lag1", "Temp_Plantation_lag1",
                      "Rainfall_Growth_lag1", "Temp_Growth_lag1",
                      "Rainfall_Maturation_lag1", "Temp_Maturation_lag1")
...
lagged_adv_vars <- lagged_df_yield %>% select(all_of(climate_variables)) %>% mutate(across(everything(), as.numeric))
...
model_df <- lagged_df_yield %>%
  select(Year, Yield,
         ...
         Total_Rainfall_Growth, Avg_Temperature_Growth,
         Total_Rainfall_Maturation, Avg_Temperature_Maturation,
         Rainfall_Plantation_lag1, Temp_Plantation_lag1,
         Rainfall_Growth_lag1, Temp_Growth_lag1,
         Rainfall_Maturation_lag1, Temp_Maturation_lag1,
         Agri_GDP_Share) %>%
...
gam_formula_parts <- c(
  ...
  "s(Rainfall_Plantation_lag1)", "s(Temp_Plantation_lag1)",
  "s(Rainfall_Growth_lag1)",     "s(Temp_Growth_lag1)",
  "s(Rainfall_Maturation_lag1)","s(Temp_Maturation_lag1)"
)
...
print(gam_summary)
capture.output(gam_summary, file = "results/gam_summary.txt")
...
par(mfrow = c(2,1))
plot(model_df$Year, model_df$gam_resid, type = "o", main = "GAM residuals over time", ylab = "Residual")
